...
static void BagMenu_ItemPrintCallback(u8 windowId, u32 itemIndex, u8 y)
{
    if (itemIndex != LIST_CANCEL)
    {
        ...
        else
        {
            ...
            if (gSaveBlock1Ptr->registeredItemR && gSaveBlock1Ptr->registeredItemR == itemSlot.itemId)
...
static void ItemMenu_Register(u8 taskId)
{
    ...
    data[1] = listPosition;
    data[2] = GetBagItemQuantity(gBagPosition.pocket + 1, listPosition);
    gSpecialVar_ItemId = GetBagItemQuantity(gBagPosition.pocket + 1, listPosition);
    sContextMenuFuncs[gBagPosition.location](taskId);
}
...
static void ItemMenu_Deselect(u8 taskId)
{
    ...
    ResetRegisteredItem(GetBagItemId(gBagPosition.pocket + 1, listPosition));
    gTasks[taskId].func = ItemMenu_FinishRegister;
}
