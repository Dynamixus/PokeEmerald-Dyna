...
GIVEN {
    ...
    PLAYER(SPECIES_MAGNEZONE){Level(64); HP(2); Ability(ABILITY_STURDY); Moves(MOVE_TACKLE, MOVE_LIGHT_SCREEN); }
...
AI_SINGLE_BATTLE_TEST("AI sees ORAORAORA killing through sturdy")
{
GIVEN {
    AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_TRY_TO_FAINT | AI_FLAG_CHECK_VIABILITY);
    PLAYER(SPECIES_MAGNEZONE){Level(64); HP(2); Ability(ABILITY_STURDY); Moves(MOVE_TACKLE, MOVE_LIGHT_SCREEN); }
    OPPONENT(SPECIES_MACHAMP_MEGA){Level(64); Moves(MOVE_DRAIN_PUNCH, MOVE_TAUNT,  MOVE_FLAMETHROWER); }
    } WHEN {
        TURN{ MOVE(player, MOVE_TACKLE);
    EXPECT_MOVE(opponent, MOVE_DRAIN_PUNCH); // AI should see drain punch as a kill due to multi hit, outscoring taunt
    }
